...
- application의 768차원, memory payload, 접근 scope와 ranking 계약을 integration API로
  밀어 넣지 않는다.
- ONNX 전환과 Qdrant 전체 재색인 동작을 보존한다.
...
1. ONNX 전환 완료 후 실제 embedder API, bundle lifecycle과 Qdrant 재색인 경계를 기준선으로 고정한다.
...
- ONNX 구현과 production vector migration을 이 작업에서 다시 수행
...
재개될 때까지 별도 모듈을 선제 생성하지 않고 `HOLD`로 둔다.
