...
예시:
- "치과 메모 찾아줘" → {"intent":"memo_search","contexts":[{"db":"memos","type":"similar","searchText":"치과"}]}
- "이번 달 일정 보여줘" → {"intent":"schedule_list","contexts":[{"db":"schedules","type":"query","filter":{"dateFrom":"2026-03-01","dateTo":"2026-03-31"}}]}
- "달걀 구매 기록" → {"intent":"grocery_add","contexts":[]}
- "안녕" → {"intent":"greeting","contexts":[]}
...
export async function analyzeIntent(
    ...
    try {
        ...
        const VALID_DBS = new Set<string>(['memos', 'todos', 'schedules', 'home_status',
            'item_locations', 'assets', 'recipes', 'grocery_items']);
        const parsed = JSON.parse(block.text.trim()) as { intent?: unknown; contexts?: unknown[] };
        const intent = typeof parsed.intent === 'string' ? parsed.intent : 'unknown';
        const contexts = Array.isArray(parsed.contexts)
            ? parsed.contexts.filter((c): c is ContextSpec =>
                c !== null && typeof c === 'object' && 'db' in c && 'type' in c &&
                VALID_DBS.has((c as { db: string }).db)
              )
            : [];
