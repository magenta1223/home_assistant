...
class KakaoMessageRepository(private val db: Database) {
    fun importMessages(messages: List<ParsedKakaoMessage>): List<KakaoMessage> = transaction(db) {
    ...
    fun listMessages(sourceFileName: String): List<KakaoMessage> = transaction(db) {
