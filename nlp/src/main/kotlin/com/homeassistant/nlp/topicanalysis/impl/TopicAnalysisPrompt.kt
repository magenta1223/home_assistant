...
internal object TopicAnalysisPrompt {
    private val retentionCriteria =
        ...
        - 더 큰 사건이나 프로젝트 topic의 claim으로 포함하는 것보다 독립적으로 둘 가치가 있습니다.

        다음은 독립 topic으로 만들지 마세요.
        - 인사, 감탄, 이모티콘, 단순한 대화 진행 표현과 미래 행동에 도움이 되지 않는 일회성 잡담
        - 답을 아직 얻지 못한 질문이나 확인 요청 하나. 관련 프로젝트의 checklist claim으로 병합하세요.
    ...
    fun system(schema: String = TopicAnalysisOutputContract.schema): String =
        ...
        claims 최대 3개 제한만을 맞추기 위해 의미적으로 같은 topic을 여러 개로 쪼개지 마세요.
        같은 topic의 세부사항이 많으면 미래 재사용성이 가장 높은 원자적 claim을 최대 3개 선택하세요.
    ...
    fun mergeSystem(schema: String = TopicAnalysisOutputContract.schema): String =
        ...
        같은 domain이라는 이유만으로 합치지 말고, 시간상 떨어져 있어도 같은 주제라면 병합하세요.
