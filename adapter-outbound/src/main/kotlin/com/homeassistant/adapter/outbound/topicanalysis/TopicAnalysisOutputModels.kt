...
internal data class TopicAnalysisLlmResponse(
    @property:SerialDescription("Proposed topics extracted from the source document. Return an empty array when there are no evidence-backed household memory topics.")
    val topics: List<TopicLlmResponse>,
)

/**
 * LLM-facing proposed topic before validation and persistence.
 *
 * @property title Short topic title generated by the LLM.
 * @property summary Concise topic summary generated by the LLM.
 * @property memoryTypes Memory categories represented by the topic.
 * @property categories Free-form category tags generated by the LLM.
...
internal data class TopicLlmResponse(
    @property:SerialDescription("Short title for one grouped household memory topic.")
    val title: String,
    @property:SerialDescription("Concise summary of why the grouped records belong together and what household memory they support.")
    val summary: String,
    ...
    @property:SerialDescription("Free-form household category tags such as housing, moving, travel, food, or finance. Categories are topical areas, not MemoryType values.")
    ...
    @property:SerialDescription("Evidence-backed canonical-memory proposals under this topic. Each memory must cite supporting source record ids.")
    val memories: List<MemoryLlmResponse>,
)

/**
 * LLM-facing proposed memory extracted under a topic.
 *
 * @property text Memory content generated by the LLM.
 ...
 * @property evidenceRecordIds Prompt-local source record ids supporting the memory.
 */
@Schema
@Serializable
internal data class MemoryLlmResponse(
    ...
    val subject: String,
    @property:SerialDescription("Allowed MemoryType enum value for this single memory. Use only enum values from the schema, never new labels such as PLAN, FACT, TODO, TASK, or COMMITMENT.")
    val memoryType: MemoryType,
    @property:SerialDescription("How directly the source evidence supports this memory: OBSERVED for direct evidence, SAID for stated content, INFERRED for cautious inference, or UNCERTAIN when unclear.")
    val certainty: MemoryCertainty,
    @property:SerialDescription("Source record ids that support this memory. Use only ids provided in the input, such as r1 or r2.")
    val evidenceRecordIds: List<String>,
)
