...
class MemoryRepository(private val db: Database) {
    ...
    fun listPending(userId: UserId, conversationId: String): List<MemoryCandidateRow> = transaction(db) {
        ensureMember(userId.value)
    ...
    fun getCandidate(id: Int): MemoryCandidateRow? = transaction(db) {
        MemoryCandidateTable.selectAll().where { MemoryCandidateTable.id eq id }
            .singleOrNull()
            ?.toCandidateRow()
    }

    fun approveCandidate(userId: UserId, candidateId: Int): MemoryRow = transaction(db) {
        ensureMember(userId.value)
    ...
    fun rejectCandidate(userId: UserId, candidateId: Int) = transaction(db) {
        ensureMember(userId.value)
    ...
    fun getMemory(id: Int): MemoryRow? = transaction(db) { fetchMemory(id) }

    fun listMemories(ids: List<Int>? = null): List<MemoryRow> = transaction(db) {
