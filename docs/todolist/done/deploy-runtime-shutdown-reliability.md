...
2026-09-06 전체 TODO 점검 중 운영 상태를 읽기 전용으로 다시 확인했다.
...
- 종료·복구 변경 commit은 현재 로컬 `master`에 포함됐지만 production은 여전히 이전 SHA를 실행한다.
  따라서 이번 확인도 기존 배포본의 정상 상태만 증명하며, 새 종료·복구 경로의 운영 검증 전까지
  `VERIFY` 상태를 유지한다.

2026-09-08 운영 검증을 완료했다.

- `ssh homeserver`의 hostname이 `HOMESERVER`임을 확인했다.
- 원격 repository HEAD와 `runtime/deploy/deployed-sha.txt`가 모두
  `89c1f0c410e2654c388382b33ee68318ba781c5c`로 일치했다.
- 이 SHA에는 종료 경쟁 처리와 실패 복구 변경이 포함되어 있으며, 자동 배포가 새 경로로 runtime을
  재기동하고 성공 SHA를 기록했다.
- 원격 `http://127.0.0.1:8080/health`가 `{"status":"ok"}`를 반환했다.

실제 자동 배포, 예약 작업 재기동, health 확인과 성공 SHA 기록까지 완료됐으므로 P0를 종료한다.
