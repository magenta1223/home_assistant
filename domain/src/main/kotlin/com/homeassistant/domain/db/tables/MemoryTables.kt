...
object FamilyTable : Table("families") {
    val id = text("id")
    val name = text("name")
...
object DomainTable : Table("domains") {
...
object ConversationMessageTable : Table("conversation_messages") {
    val id = integer("id").autoIncrement()
    val familyId = text("family_id").references(FamilyTable.id)
...
object MemoryCandidateTable : Table("memory_candidates") {
    val id = integer("id").autoIncrement()
    val familyId = text("family_id").references(FamilyTable.id)
    val conversationId = text("conversation_id")
...
object MemoryTable : Table("memories") {
    val id = integer("id").autoIncrement()
    val familyId = text("family_id").references(FamilyTable.id)
    val domainId = integer("domain_id").references(DomainTable.id)
    val memoryKind = text("memory_kind")
