...
import com.homeassistant.application.memory.save.MemoryIndexingCoordinator
...
class MemoryPlacementService(
    ...
    private val indexing = MemoryIndexingCoordinator(
        memories = memories,
        indexWriter = memoryIndexWriter,
        outbox = indexingOutbox,
    )
    ...
    private suspend fun placeBatch(userId: UserId, batch: List<Memory>) {
        ...
        val structuralRootIds = memories.findRootMemories()
        ...
        val inputs = batch.map { memory ->
            val rootHits = searcher.search(
                ...
                scope = MemoryIndexSearchScope(
                    allowedMemoryIds = structuralRootIds,
                ),
            )
    ...
    private fun findStructuralCandidates(
        query: String,
        rootHits: List<com.homeassistant.application.memory.io.MemoryIndex>,
    ): List<Memory> {
        val roots = memories.findByIds(rootHits.map { it.memoryId })
            .filter { it.visibility == MemoryVisibility.STRUCTURAL }
            .associateBy { it.id }
        return rootHits
