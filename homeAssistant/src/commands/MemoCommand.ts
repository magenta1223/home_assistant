...
export class MemoCommand extends SharedableCommand {
    register(app: App): void {
        app.command('/메모', async ({ command, ack, respond }) => {
            await ack();
            await respond(this.addMemo(command.text.trim(), command.user_id) as never);
        });

        app.command('/메모목록', async ({ command, ack, respond }) => {
            await ack();
            await respond(this.listMemos(command.text.trim(), command.user_id) as never);
        });

        app.command('/메모검색', async ({ command, ack, respond }) => {
            await ack();
            await respond(this.searchMemos(command.text.trim(), command.user_id) as never);
        });
    }

    private addMemo(text: string, userId: string): SlackResponse {
        if (!text) return this.err('내용을 입력해주세요. 예: `/메모 치과 예약 내일 3시`');
    ...
    private searchMemos(query: string, userId: string): SlackResponse {
        if (!query) return this.err('검색어를 입력해주세요. 예: `/메모검색 치과`');
